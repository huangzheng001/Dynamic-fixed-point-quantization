...
class vgg_quantization(nn.Module):
    ...
    def forward(self, x):
        ...
        ##################################################
        x = self.conv3(x)
        x = quantization_F.apply(x, bit_width, bit_width - int_blob['conv3'])
        x = self.conv4(x)
        x = quantization_F.apply(x, bit_width, bit_width - int_blob['conv4'])
        x = nn.MaxPool2d(kernel_size=2, stride=2)(x)
        ####################################
        x = self.conv5(x)
        x = quantization_F.apply(x, bit_width, bit_width - int_blob['conv5'])
        x = self.conv6(x)
        x = quantization_F.apply(x, bit_width, bit_width - int_blob['conv6'])
        x = self.conv7(x)
        x = quantization_F.apply(x, bit_width, bit_width - int_blob['conv7'])
        x = nn.MaxPool2d(kernel_size=2, stride=2)(x)
        #########################################
        x = self.conv8(x)
        x = quantization_F.apply(x, bit_width, bit_width - int_blob['conv8'])
        x = self.conv9(x)
        x = quantization_F.apply(x, bit_width, bit_width - int_blob['conv9'])
        x = self.conv10(x)
        x = quantization_F.apply(x, bit_width, bit_width - int_blob['conv10'])
        x = nn.MaxPool2d(kernel_size=2, stride=2)(x)
        ############################################
        x = self.conv11(x)
        x = quantization_F.apply(x, bit_width, bit_width - int_blob['conv11'])
        x = self.conv12(x)
        x = quantization_F.apply(x, bit_width, bit_width - int_blob['conv12'])
        x = self.conv13(x)
        x = quantization_F.apply(x, bit_width, bit_width - int_blob['conv13'])
        x = nn.MaxPool2d(kernel_size=2, stride=2)(x)
...
optimizer = optim.SGD(model2.parameters(), lr=0.1, momentum=0.9, weight_decay=1e-4)
best_prec1 = 0.
epochs = 160 #200
...
for epoch in range(0, epochs):
    ...
    print('\nTest set: Average loss: {:.4f}, Accuracy: {}/{} ({:.1f}%)\n'.format(
        ...
        100. * correct / len(test_loader.dataset)))
    # if epoch%40==0:
    #     print('{}/{}, {}'.format(epoch, epochs, epoch/epochs))
    best_prec1 = max(best_prec1, 100. * correct / len(test_loader.dataset))

print('bit width: {},  best: {}'.format(bit_width, best_prec1))